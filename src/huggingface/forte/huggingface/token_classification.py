    ...
    def _process(self, input_pack: DataPack):
        ...
        for entry_specified in input_pack.get(entry_type=input_entry):
            ...
            if self.configs.strategy == 'bio-merge':
                result = self._merge_bio_tokens(result)

            for idx, token in enumerate(result):
                # entity = token['entity']

                NE = token['NE'] if "NE" in token else None

                start, end = token['start'], token['end']

                entity = output_entry(pack=input_pack, begin=start, end=end)

                if self.configs.attribute_name == "ner":
                    if output_module_str == "EntityMention":
                        entity.ner_type = NE if NE else token['entity']
                    elif output_module_str == "Token":
                        entity.ner = NE if NE else token['entity']
                    else:
                        warnings.warn(f'NER type was not stored in the given '
                ...
                if self.configs.attribute_name == "pos":
                    if output_module_str == "Token":
                        entity.pos = token['entity']
                    else:
                        warnings.warn(f'POS type was not stored in the given '
                                      f'\'output_entry_type\': {self.configs.output_entry_type}.'
